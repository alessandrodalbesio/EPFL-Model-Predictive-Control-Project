classdef NmpcControl < handle
    ...
    methods
        function obj = NmpcControl(rocket, tf, MAX_ROLL_15DEG)
            ...
            if MAX_ROLL_15DEG
                Qref = diag([1,1,1,1]);
                Qnoref = diag([1,1,1,1,1,1,1,1]);
                R = diag([1,1,1,1]);
            else
            end
            id_ref = [6,10,11,12];
            id_no_ref = setdiff(1:12,id_ref);
            Q = zeros(12,12);
            Q(id_ref,id_ref) = Qref;
            Q(id_no_ref,id_no_ref) = Qnoref;

            % Define constraints
            ubx(5) = deg2rad(75);
            lbx(5) = deg2rad(-75);
            ubu(1:4) = [0.1745;0.1745;80;20];
            lbu(1:4) = [-0.1745;-0.1745;50;-20];
            ...
            for i=1:N-1
                ...
                cost = cost + (X_sym(id_ref,i)-ref_sym)'*Qref*(X_sym(id_ref,i)-ref_sym) + X_sym(id_no_ref,i)'*Qnoref*X_sym(id_no_ref,i)+ U_sym(:,i)'*R*U_sym(:,i);
            end
            ...
            cost = cost + (X_sym(id_ref,N)-ref_sym)'*Qf(id_ref,id_ref)*(X_sym(id_ref,N)-ref_sym) + X_sym(id_no_ref,N)'*Qf(id_no_ref,id_no_ref)*X_sym(id_no_ref,N);
